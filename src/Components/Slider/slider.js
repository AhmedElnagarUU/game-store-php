...
const Slider = props => {
  ...
  // let fruits = selectedGame.footage.split(",")
let slideimage=[]
  for (let index = 0; index < allGames.length; index++) {
    const element = allGames[index];
    
    // التحقق من وجود اسم اللعبة في localStorage
    const storedName = window.localStorage.getItem("name");
  
 
    
    if (element.surname === storedName) {

      // تقسيم footage إلى مصفوفة باستخدام الفاصلة
      const footageArray = element.footage.split("@");
   slideimage=footageArray
   console.log(slideimage);
   
      // يمكنك الآن استخدام footageArray هنا
